...
second_ref = False
reference2_map_file = 'emd_2984_from_pdb.mrc'
# output directory for slices
dirout = 'EM_slices_1axis_3D'
# index file for output slices
index_file = 'EM_slices_1axis_3D.idx'
...
for axis in axes_list:
  ...
  for section in range(0,loc_mrcdata.shape[2]-window_size,section_skip):
    ...
    for col in range(0,loc_mrcdata.shape[1]-window_size,offset):
        ...
        for row in range(0,loc_mrcdata.shape[0]-window_size,offset):
            # adding in ":section+window_size" seems to add in the extra dimention to make it a 3d slice 
            myslice = loc_mrcdata[row:row+window_size,col:col+window_size,section:section+window_size].copy(order='C')
            ...
            imgout_filename = annotate + '2984_bl_'+str(section)+'_'+str(col)+'_'+str(row)+axis+'.npy'
            ...
            out_filename = dirout + '/' + imgout_filename
            np.save(out_filename,myslice)
